...
(()=>{'use strict';
const client=()=>window.qfSupabaseClient||window.supabaseClient||(window.supabase?.createClient?null:null);
...
async function view(id){
 if(!id)return;
 document.getElementById('qfAdminUserModal')?.remove();
 const modal=document.createElement('div');modal.id='qfAdminUserModal';modal.className='admin-user-modal';
 ...
 try{
  ...
  const habitHtml=habits.length?habits.map(h=>{const dates=Array.isArray(h.recent_completions)?h.recent_completions:[],target=Number(h.target_per_week||0),count=Number(h.completion_count||0);return `<article class="admin-habit"><div class="admin-habit-top"><div><h4>${esc(h.icon||'🌱')} ${esc(h.name||'Untitled habit')}</h4><div class="muted">${esc(h.description||'No description')} • ${esc(h.frequency||'Custom')}</div></div><span class="badge">${h.is_active?'Active':'Paused'}</span></div><div class="admin-target-grid"><div><span>Weekly target</span><strong>${target}×</strong></div><div><span>Total completions</span><strong>${count}</strong></div></div><div class="muted" style="margin-top:8px">${h.reminder_time?`⏰ Reminder ${esc(String(h.reminder_time).slice(0,5))}`:'⏰ No reminder set'}</div>${dates.length?`<div class="admin-completion-list">${dates.slice(0,12).map(d=>`<span class="admin-date-pill">✓ ${fmt(d)}</span>`).join('')}</div>`:'<div class="muted small" style="margin-top:8px">No completions recorded.</div>'}</article>`}).join(''):'<div class="admin-empty">🌱 This user has not created any habits.</div>';
  modal.querySelector('.admin-user-body').innerHTML=`<div class="admin-user-stats"><div class="admin-user-stat"><span class="muted">⚡ XP</span><strong>${Number(p.xp||0).toLocaleString()}</strong></div><div class="admin-user-stat"><span class="muted">🎯 Goals</span><strong>${goals.length}</strong></div><div class="admin-user-stat"><span class="muted">🌱 Active habits</span><strong>${active}</strong></div><div class="admin-user-stat"><span class="muted">🧩 Milestones</span><strong>${completedMilestones}/${milestones}</strong></div></div><section class="admin-user-section"><div class="admin-user-section-head"><div><h3>🎯 Goals, targets & milestones</h3><div class="muted">${completedGoals}/${goals.length} goals completed • ${completedMilestones}/${milestones} milestones completed</div></div></div><div class="admin-user-list">${goalHtml}</div></section><section class="admin-user-section"><div class="admin-user-section-head"><div><h3>🌱 Habit tracker & targets</h3><div class="muted">Current habits, weekly targets and recent completion history • ${habitTarget} total weekly target check-ins</div></div></section><section class="admin-user-section"><div class="muted">Account created ${fmt(p.created_at)} • Last profile update ${fmt(p.updated_at)} • Leaderboard ${p.leaderboard_visible===false?'hidden':'visible'}</div></section>`;
...
(()=>{'use strict';
 ...
 const cards=[['🌱','Total habits','habits_total','All habits created'],['📚','Total sessions','study_sessions_total','All study sessions'],['🎯','Total goals','goals_total','All goals created'],['🧩','Total milestones','milestones_total','All milestones created']];
 let lastKey='';
