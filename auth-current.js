...
(() => {
  ...
  let mode = 'signin';
  let lastRoot = null;
  const icon = n => ({mail:'<svg viewBox="0 0 24 24"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="m3 7 9 6 9-6"/>',lock:'<svg viewBox="0 0 24 24"><rect x="4" y="10" width="16" height="11" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/>',eye:'<svg viewBox="0 0 24 24"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="2.5"/></svg>'}[n]||'');
  ...
  window.qfLogin=async()=>{const e=normalizeEmail(document.getElementById('email')?.value),p=document.getElementById('password')?.value||'',validation=validateEmail(e),s=document.getElementById('authStatus');if(validation)return toast(validation);if(!p)return toast('Enter your password');if(p.length<6)return toast('Password must be at least 6 characters');if(s)s.textContent='Signing you in…';try{const r=await supabaseClient.auth.signInWithPassword({email:e,password:p});if(r.error){if(s)s.textContent='';const msg=r.error.message||'';if(/email.*not.*confirm|not.*verified|confirm.*email/i.test(msg))return toast('Please verify your email before logging in.');return toast(msg)}await bootstrap()}catch(x){if(s)s.textContent='';toast(x?.message||'Could not sign in.')}};
  window.qfCreateAccount=async()=>{const now=Date.now();if(now<lockedUntil)return toast(`Please wait ${Math.ceil((lockedUntil-now)/1000)}s before trying again.`);const e=normalizeEmail(document.getElementById('email')?.value),p=document.getElementById('password')?.value||'',s=document.getElementById('authStatus'),validation=validateEmail(e);if(validation)return toast(validation);if(p.length<6)return toast('Password must be at least 6 characters');lockedUntil=now+WAIT_MS;if(s)s.textContent='Creating your account…';try{const r=await Promise.race([supabaseClient.auth.signUp({email:e,password:p}),new Promise((_,reject)=>setTimeout(()=>reject(new Error('Request timed out. Please check your internet connection.')),10000))]);if(r.error){lockedUntil=0;if(s)s.textContent=r.error.message;return toast(r.error.message)}const session=r.data?.session;if(session){lockedUntil=0;if(s)s.textContent='Account created successfully.';return bootstrap()}const current=await supabaseClient.auth.getSession();if(current.data?.session){lockedUntil=0;if(s)s.textContent='Account created successfully.';return bootstrap()}lockedUntil=0;if(s)s.textContent='Account created. Please check your email to verify your account before logging in.';return toast('Account created. Check your email to verify your account.')}catch(x){lockedUntil=0;if(s)s.textContent=x?.message||'Could not create the account.';toast(x?.message||'Could not create the account.')}};
